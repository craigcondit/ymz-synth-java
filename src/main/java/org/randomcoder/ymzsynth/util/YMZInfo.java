    ...
    Files.walkFileTree(new File(dir).toPath(), new SimpleFileVisitor<Path>() {
      ...
      public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
        ...
        if (attrs.isRegularFile() && file.getFileName().toString()
            .toLowerCase(Locale.US).endsWith(".ymz")) {
          ...
          long registersSec =
              timeInSeconds > 0 ? (registerCount / timeInSeconds) : 0;
          long clockSec = timeInSeconds > 0 ? (clockCount / timeInSeconds) : 0;

          String msg =
              file.toString() + ": registers=" + registerCount + " clocks="
                  + clockCount + " bad_registers=" + badRegisterCount
                  + " time_in_sec=" + timeInSeconds + " registers/sec="
                  + registersSec + " clocks/sec=" + clockSec;
